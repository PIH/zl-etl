...
select  hiv_vl_id,
ROW_NUMBER() over (PARTITION by emr_id order by vl_sample_taken_date ASC, iif(vl_coded_results is null, 0,1) ASC, vl_result_date ASC, hiv_vl_id ASC ) "index_asc",
ROW_NUMBER() over (PARTITION by emr_id order by vl_sample_taken_date DESC, iif(vl_coded_results is null, 0,1) DESC, vl_result_date DESC, hiv_vl_id DESC) "index_desc"
...
from hiv_viral_load av
inner join #hiv_viral_load_indexes avi on av.hiv_vl_id = avi.hiv_vl_id;
...
-- update program index asc/desc on hiv_visit table
drop table if exists #hiv_visit_indexes;
select  emr_id, visit_date, encounter_id,
...
-- update program index asc/desc on pmtct_visits table
drop table if exists #pmtct_visits_indexes;
select  emr_id, visit_date,visit_id,encounter_id,
