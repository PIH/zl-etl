...
insert into hiv_patient_summary_status (emr_id)
...
set legacy_emr_id = hp.hivemr_v1_id
    from hiv_patient_summary_status t
...
inner join 
	(SELECT emr_id, STUFF(
	         (SELECT DISTINCT ',' + drug_short_name
	          FROM hiv_regimens r
   	          inner join hiv_patient_summary_status t2 on t2.emr_id = r.emr_id and t2.arv_start_date = format(r.start_date  , 'd') 
...
inner join 
	(SELECT emr_id, STUFF(
	         (SELECT DISTINCT ',' + drug_short_name
	          FROM hiv_regimens r
--   	          inner join hiv_patient_summary_status t on t.emr_id = r.emr_id and t.arv_start_date = r.start_date 
	          WHERE r.emr_id = r2.emr_id
	          and order_action = 'NEW'	
			  and drug_category = 'ART'
