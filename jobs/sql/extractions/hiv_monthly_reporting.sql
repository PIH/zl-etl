...
latest_arv_regimen_order_id		INT(11),
latest_arc_regimen_encounter_id	INT(11),
...
create temporary table temp_last_arv_order_id 
	select t.monthly_reporting_id , o.order_id, o.date_activated, o.encounter_id  
	from temp_eom_appts t 
	inner join 
		(select o2.order_id, o2.patient_id ,  o2.date_activated, o2.encounter_id  
		from orders o2 
		where o2.voided = 0
		and o2.order_reason = concept_from_mapping('CIEL','138405')
		order by if(o2.date_stopped is null,0,1) asc, o2.date_activated desc) o on o.patient_id = t.patient_id and o.date_activated < t.reporting_date  
...
set latest_arv_regimen_order_id = tlao.order_id,
	latest_arv_regimen_date = tlao.date_activated,
	latest_arc_regimen_encounter_id = tlao.encounter_id ;

update temp_eom_appts t 
set latest_arv_regimen_line = obs_value_coded_list(t.latest_arc_regimen_encounter_id, 'PIH','13115',@locale );
...
create temporary table temp_last_arv_dispensing_id 
	...
	inner join 
		...
		from encounter e2
		inner join obs o on o.voided = 0 and o.encounter_id = e2.encounter_id and  o.concept_id =  concept_from_mapping('PIH','1535')
			and o.value_coded in (concept_from_mapping('PIH','3013'),concept_from_mapping('PIH','2848'),concept_from_mapping('PIH','13960'))
		where e2.voided = 0
