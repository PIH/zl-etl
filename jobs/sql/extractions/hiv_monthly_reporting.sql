...
-- ############################### HIV Visit Data ##################################################################
UPDATE temp_eom_appts
SET temp_eom_appts.latest_hiv_visit_date = x.visit_date
...
UPDATE temp_eom_appts
SET 
temp_eom_appts.latest_expected_hiv_visit_date =av.next_visit_date,
temp_eom_appts.hiv_visit_days_late=DATEDIFF(DAY,isnull(av.next_visit_date,isnull(t1.latest_hiv_visit_date,t1.date_enrolled)),t1.reporting_date)
FROM  temp_eom_appts t1 
...
UPDATE temp_eom_appts
SET temp_eom_appts.latest_dispensing_date = x.dispense_date
FROM  temp_eom_appts t1 
LEFT OUTER JOIN 
(
...
UPDATE temp_eom_appts
SET 
temp_eom_appts.latest_expected_dispensing_date=ad.next_dispense_date,
temp_eom_appts.dispensing_days_late=DATEDIFF(DAY,isnull(ad.next_dispense_date,isnull(t1.latest_dispensing_date,t1.date_enrolled)),t1.reporting_date)
FROM  temp_eom_appts t1 
...
UPDATE temp_eom_appts
SET temp_eom_appts.latest_hiv_viral_load_date = x.vl_sample_taken_date
FROM  temp_eom_appts t1 
LEFT OUTER JOIN 
(
...
UPDATE temp_eom_appts
SET temp_eom_appts.latest_hiv_viral_load_coded = avl.vl_coded_results,
temp_eom_appts.latest_hiv_viral_load=avl.viral_load
FROM  temp_eom_appts t1 
...
UPDATE temp_eom_appts
SET temp_eom_appts.latest_arv_regimen_date = x.encounter_datetime
FROM  temp_eom_appts t1 
LEFT OUTER JOIN 
(
...
UPDATE temp_eom_appts
SET temp_eom_appts.latest_arv_regimen_line = r.art_treatment_line
FROM  temp_eom_appts t1 
...
UPDATE temp_eom_appts
SET temp_eom_appts.latest_arv_dispensed_date = x.dispense_date
FROM  temp_eom_appts t1 
LEFT OUTER JOIN 
(
...
UPDATE temp_eom_appts
SET temp_eom_appts.latest_arv_dispensed_line = ad.current_art_treatment_line
FROM  temp_eom_appts t1 
